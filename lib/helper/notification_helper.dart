...
import 'package:anythingz/common/widgets/demo_reset_dialog_widget.dart';
import 'package:anythingz/common/widgets/taxi_make_payment_bottomsheet.dart';
import 'package:anythingz/features/chat/controllers/chat_controller.dart';
import 'package:anythingz/features/chat/enums/user_type_enum.dart';
...
import 'package:anythingz/features/rental_module/rental_order/screens/taxi_order_details_screen.dart';
...
import 'package:anythingz/features/notification/widgets/notifiation_popup_dialog_widget.dart';
...
class NotificationHelper {
    ...
    flutterLocalNotificationsPlugin.initialize(initializationsSettings, onDidReceiveNotificationResponse: (NotificationResponse load) async {
      try{
        if (load.payload?.isNotEmpty ?? false) {
          final Map<String, dynamic> data = jsonDecode(load.payload!);
          Map<String, dynamic> normalizedData = Map<String, dynamic>.from(data);
          if (data.containsKey('notificationBody') && data['notificationBody'] is Map) {
            normalizedData.addAll(Map<String, dynamic>.from(data['notificationBody']));
          }

          final NotificationBodyModel notificationBody = convertNotification(normalizedData);

          if (normalizedData.containsKey('store_id') &&
              normalizedData['store_id'] != null &&
              normalizedData['store_id'].toString().isNotEmpty) {
            final storeId = int.tryParse(normalizedData['store_id'].toString());
            if (storeId != null) {
          ...
          final Map<NotificationType, Function> notificationActions = {
            NotificationType.order: () {
              if(AuthHelper.isGuestLoggedIn()) {
                ...
                Get.toNamed(RouteHelper.getOrderDetailsRoute(int.parse(notificationBody.orderId.toString()), fromNotification: true));
              }
            },
            ...
            NotificationType.trip: () => Get.to(()=> TaxiOrderDetailsScreen(tripId: int.parse(notificationBody.orderId.toString()))),
          };
    ...
    FirebaseMessaging.onMessageOpenedApp.listen((RemoteMessage message) async {
      ...
      try{
        if(message.data.isNotEmpty) {
          ...
          if (data.containsKey('store_id') && data['store_id'] != null && data['store_id'].toString().isNotEmpty) {
            final storeId = int.tryParse(data['store_id'].toString());
            if (storeId != null) {
              final storeController = Get.find<StoreController>();
              ...
              if (store != null) {
                ...
                return; // ✅ Stop further routing since we handled it
              }
            }
          }
          ...
          final Map<NotificationType, Function> notificationActions = {
            ...
            NotificationType.store: () => Get.toNamed(RouteHelper.getStoreRoute(id: int.parse(message.data['restaurantId']), page: 'item')),
          };
  ...
  static Future<void> showNotification(RemoteMessage message, FlutterLocalNotificationsPlugin fln) async {
    if(!GetPlatform.isIOS) {
      ...
      // storeId = message.data['store_id'];

      // if (message.data['type'] == 'navigate_Store') {
      //   storeId = int.tryParse(message.data['restaurantId'] ?? '0');
      //   notificationBody = NotificationBodyModel(
      //     notificationType: NotificationType.store,
      //     restaurantId: storeId,
      //   );
      // }//ADDED BY SAITEJA ON 18-08-2025
      ...
      if(image != null && image.isNotEmpty) {
        try{
          ...
          await showBigPictureNotificationHiddenLargeIcon(title, body, orderID, notificationBody, image, fln, message.data);

        }catch(e) {
          // await showBigTextNotification(title, body!, orderID, notificationBody, fln);
          await showBigTextNotification(title, body!, orderID, notificationBody, fln, message.data);
        }
      }else {
        // await showBigTextNotification(title, body!, orderID, notificationBody, fln);
        await showBigTextNotification(title, body!, orderID, notificationBody, fln, message.data);
  ...
  // static Future<void> showBigTextNotification(String? title, String body, String? orderID, NotificationBodyModel? notificationBody, FlutterLocalNotificationsPlugin fln) async {
    static Future<void> showBigTextNotification(String? title, String body, String? orderID, NotificationBodyModel? notificationBody, FlutterLocalNotificationsPlugin fln, Map<String, dynamic>? data) async {
    ...
    await fln.show(0, title, body, platformChannelSpecifics,
      // payload: jsonEncode({
      //   ...message.data, // preserve the full data
      //   'notificationBody': notificationBody?.toJson(), // keep structured info too
      // }),
      payload: jsonEncode({
        ...?data,
        'notificationBody': notificationBody?.toJson(),
      }),
    );
  }

  static Future<void> showBigPictureNotificationHiddenLargeIcon(String? title, String? body, String? orderID, NotificationBodyModel? notificationBody, String image, FlutterLocalNotificationsPlugin fln, Map<String, dynamic>? data) async {
    ...
    if (notificationBody != null) payloadData['notificationBody'] = notificationBody.toJson();
    ...
    switch (type) {
      ...
      case 'navigate_Store':
        return NotificationBodyModel(
          notificationType: NotificationType.store,
          restaurantId: int.tryParse(data['store_id'] ?? '0'),
        );
      default:
        return NotificationBodyModel(notificationType: NotificationType.general);
    }
  ...
  static NotificationBodyModel _handleStoreNotification(Map<String, dynamic> data) {
    final storeId = data['id'];
    return NotificationBodyModel(
      orderId: int.tryParse(storeId) ?? 0,
      notificationType: NotificationType.order,
    );
  }
