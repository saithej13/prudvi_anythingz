...
import 'package:anythingz/api/api_checker.dart';
import 'package:anythingz/features/address/domain/models/address_model.dart';
import 'package:anythingz/common/models/error_response.dart';
...
class ApiClient extends GetxService {
  ...
  final int timeoutInSeconds = 20; // Reduced from 40s to 20s for better UX
  ...
  Map<String, String> updateHeader(String? token, List<int>? zoneIDs, List<int>? operationIds, String? languageCode, int? moduleID, String? latitude, String? longitude, {bool setHeader = true}) {
    ...
    print('moduleId print by saiteja: ' + moduleID.toString());
    header.addAll({
  ...
  Future<Response> _retryRequest(
    ...
    while (retryCount <= maxRetries) {
      try {
        ...
        // Only retry on timeout (408) or connection errors (1)
        if (response.statusCode != 408 && response.statusCode != 1) {
          return response;
        }

        if (retryCount < maxRetries) {
          if (kDebugMode) {
            print('Retry attempt ${retryCount + 1}/$maxRetries after ${delay.inSeconds}s delay');
          }
        ...
        if (kDebugMode) {
          print('Retry attempt ${retryCount + 1}/$maxRetries after ${delay.inSeconds}s delay (error: $e)');
        }
        await Future.delayed(delay);
        delay *= 2;
        retryCount++;
  ...
  Future<Response> getData(String uri, {Map<String, dynamic>? query, Map<String, String>? headers, bool handleError = true, bool enableRetry = false}) async {
    if (enableRetry) {
      return _retryRequest(() => _performGetRequest(uri, headers, handleError));
    }
    return _performGetRequest(uri, headers, handleError);
  }

  Future<Response> _performGetRequest(String uri, Map<String, String>? headers, bool handleError) async {
    try {
      if (kDebugMode) {
      ...
      if (kDebugMode) {
        print('------------TimeoutException on $uri: ${e.toString()}');
      }
      return Response(statusCode: 408, statusText: 'request_timeout'.tr);
    } catch (e) {
      if (kDebugMode) {
        print('------------Error on $uri: ${e.toString()}');
      }
      return Response(statusCode: 1, statusText: noInternetMessage);
    }
  }
