...
import 'package:anythingz/features/cart/domain/models/online_cart_model.dart';
  ...
  double calculationCart() {
    ...
    for (var cartModel in cartList) {
      ...
      if(_variationPrice>0){
        double basePrice = _variationPrice;
        double? variationPrice = 0;

        double actualPrice = (variationPrice != null && variationPrice > 0)
            ? variationPrice
            : basePrice;

        variationWithoutDiscountPrice =0;

        double price = actualPrice * cartModel.quantity!;
        double discountPrice = haveVariation ? (variationWithoutDiscountPrice - _variationPrice)
            :  (PriceConverter.convertWithDiscountNew(price, discount, discountType)!);
        ...
        double basePrice = cartModel.item!.price!;
        double? variationPrice = haveVariation ? variationWithoutDiscountPrice : 0;
