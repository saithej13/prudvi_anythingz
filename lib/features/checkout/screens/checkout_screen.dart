...
import 'package:anythingz/features/cart/domain/models/cart_model.dart';
import 'package:anythingz/common/models/config_model.dart';
import 'package:anythingz/features/location/domain/models/zone_response_model.dart';
...
import 'package:anythingz/helper/price_converter.dart';
...
import 'package:anythingz/common/widgets/custom_button.dart';
import 'package:anythingz/common/widgets/custom_dropdown.dart';
...
import 'package:anythingz/features/checkout/widgets/checkout_screen_shimmer_view.dart';
import 'package:anythingz/features/checkout/widgets/payment_method_bottom_sheet.dart';
import 'package:get/get.dart';
import 'package:anythingz/features/checkout/widgets/bottom_section.dart';
import 'package:anythingz/features/checkout/widgets/top_section.dart';
...
class CheckoutScreenState extends State<CheckoutScreen> {
    ...
    return Scaffold(
      ...
      body: guestCheckoutPermission || AuthHelper.isLoggedIn() ? GetBuilder<CheckoutController>(builder: (checkoutController) {
        ...
        return GetBuilder<CouponController>(builder: (couponController) {
          ...
          if(moduleData != null) {
            /*
            if(order.store!.maximum_cod_order_amount != null && order.store!.maximum_cod_order_amount! > 0) {
                      _maxCodOrderAmount = order.store!.maximum_cod_order_amount;
                    } else {
                      _maxCodOrderAmount = m.pivot!.maximumCodOrderAmount;
                    }
                    break;
            * */
            if(checkoutController.store != null && checkoutController.store!.maximum_cod_order_amount != null
                && checkoutController.store!.maximum_cod_order_amount! > 0) {
              maxCodOrderAmount = checkoutController.store!.maximum_cod_order_amount;
            } else {
          ...
          double? discount = _calculateDiscount(
            store: checkoutController.store, cartList: _cartList, price: price, addOns: addOns,variation: variations
          );
  ...
  double _calculateDiscount({required Store? store, required List<CartModel?>? cartList, required double price, required double addOns,required double variation}) {
    double discount = 0;
    double variationPrice = 0;
    double variationDiscount = 0;
    ...
    if (cartList != null) {
      for (var cartModel in cartList) {
        ...
        if(isFoodVariation) {
          // discount = cartModel!.item!.storeDiscount == 0 ? cartModel.item!.discount : cartModel.item!.storeDiscount;
          String? discountType = cartModel.item!.storeDiscount == 0 ? cartModel.item!.discountType : 'percent';
          ...
          if(Get.find<SplashController>().getModuleConfig(cartModel.item!.moduleType).newVariation!) {
            ...
            // double d = ((cartModel.item!.price! - PriceConverter.convertWithDiscount(variation, dis, disType)!));
            // discount = discount + d;
            // if(disType == 'percent' && discount != 0) {
            //   discount = discount + _calculateFoodVariationDiscount(cartModel: cartModel);
            // }
            //discount == (discount! + (variationDiscount - variationPrice));
            discount = ((dis! * (variationDiscount))/100);
          }
        }
         else {
          String variationType = '';
          ...
          if(cartModel.item!.variations!.isNotEmpty){
            for (Variation variation in cartModel.item!.variations!) {
              if (variation.type == variationType) {
                ...
                variationPrice += (PriceConverter.convertWithDiscount(variation.price!, dis, disType)!);
  ...
  //           // }
  //           //discount == (discount! + (variationDiscount - variationPrice));
  //           discount = (discount! * (variationDiscount));
  //         }
  //       }
  //       else {
  //         String variationType = '';
  ...
  double _calculateOrderAmount({required double price, required double variations, required double discount, required double addOns, required double couponDiscount, required List<CartModel?>? cartList, required double referralDiscount}) {
    ...
    if(variationPrice>0){
      orderAmount = (variationPrice - discount) + addOns - couponDiscount - referralDiscount;
    }
    else{
      orderAmount = (price - discount) + addOns - couponDiscount - referralDiscount;
    }
    // orderAmount = (price + variationPrice - discount) + addOns - couponDiscount - referralDiscount;
    //UPDATED BY SAITEJA ON 10-DEC-2025
